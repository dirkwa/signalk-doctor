 ...
 * (which is what it does when loaded standalone at :3004).
 *
...
interface ProxyOptions {
  /** Returns the engine base URL, e.g. "http://localhost:3004". Re-read on every request so config changes take effect without restart. */
  getTargetUrl: () => string;
  /** Public-facing path prefix where this proxy is mounted, used for the api-base meta tag (e.g. "/plugins/signalk-doctor/console"). */
...
export function createConsoleProxy(opts: ProxyOptions) {
  return (req: Request, res: Response): void => {
    ...
    // "/plugins/signalk-doctor/console/foo" mounted at "/console", req.url
    ...
    proxyReq.on('error', (err) => {
      ...
      if (!res.headersSent) {
        res
          ...
          .type('text/plain')
          .send(
            `Doctor console at ${target.origin} is not reachable: ${errMsg(err)}\n` +
              `Check: systemctl --user status signalk-doctor-server.service`,
          );
      } else {
        res.end();
      }
