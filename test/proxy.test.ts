...
describe('injectApiBaseMeta', () => {
  it('inserts the meta tag right after <head>', () => {
    ...
    const out = injectApiBaseMeta(html, '/plugins/signalk-doctor/console');
    expect(out).toContain('<meta name="api-base" content="/plugins/signalk-doctor/console">');
...
describe('createConsoleProxy', () => {
  ...
  async function makeApp(): Promise<{
    ...
    const proxy = createConsoleProxy({
      ...
      publicPathPrefix: '/plugins/signalk-doctor/console',
    });
  ...
  it('injects the api-base meta tag into HTML responses', async () => {
    ...
    try {
      ...
      expect(html).toContain('<meta name="api-base" content="/plugins/signalk-doctor/console">');
    } finally {
      await close();
    }
  });
